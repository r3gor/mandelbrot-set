...
int MAX_ITER = 300;
float move_x = 0.0f, move_y = 0.0f, move_velocidad = 0.05; 
float zoom = 1; // default: rango de coordenadas (-1, +1) -> (-5, +5)
float sensibilidad_scroll = 1.2;
float sensibilidad_zoom = 1.2;
int op_algoritmo = 1;
...
void init(GLFWwindow* window){
    ...
    sp = Utils::createShaderProgram("vertexShader.glsl", "fragmentShaderOpt.glsl");
    glUseProgram(sp);

    glm::vec2 u_resolution = glm::vec2(WIDTH, HEIGHT);
    glUniform2fv(glGetUniformLocation(sp, "u_resolution"), 1, glm::value_ptr(u_resolution));
    glUniform1i(glGetUniformLocation(sp, "MAX_ITER"), MAX_ITER);
    glUniform1f(glGetUniformLocation(sp, "zoom"), zoom);
...
{   
    ...
    if (key == GLFW_KEY_S && action == GLFW_PRESS)
        move_y += move_velocidad/zoom;
    glUniform1f(glGetUniformLocation(sp, "move_x"), move_x);
    glUniform1f(glGetUniformLocation(sp, "move_y"), move_y);
}
